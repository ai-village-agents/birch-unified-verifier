...
This module implements the structured 5-pass checklist used for the
BIRCH v0.3 unified spec review.  It also optionally attaches results
from external helper tools (DeepSeek's PR verifier, GPT-5.2's diff
scanner, and GPT-5.1's encoding-scan helper) as a sidecar under the
``external_tools`` key in the returned result.
"""
...
class FivePassVerifier:
    """Execute the structured 5-pass verification.

    The main method :meth:`execute_five_pass` returns a dictionary with
    pass-by-pass details based on parsing the PR content and diff.  When
    external integrations are enabled it also runs the helper tools and
    records their outputs.
    """
    ...
    def __init__(
        ...
        self.parser: PRContentParser | None = None

        # Optional sidecar integrations (DeepSeek verifier, GPT-5.2
        # scanner, and encoding-scan helper).
        self.enable_external_tools = enable_external_tools
        self.spec_path = spec_path

    def execute_five_pass(
        self,
        pr_url: str,
        enable_external_tools: Optional[bool] = None,
        spec_path: Optional[str] = None,
    ) -> Dict[str, Any]:
        ...
        ----------
        pr_url:
            URL of the PR to verify.
        enable_external_tools:
            If True, also run the optional external helper tools.  If
            ``None``, falls back to the value configured at
            construction-time.
        spec_path:
            Optional path to the unified spec file to hand to the
            encoding-scan helper.
        """

        # Prepare PR content parser (used by several passes).
        self.parser = PRContentParser(pr_url)
        self.parser.fetch_pr_content()
        self.parser.fetch_pr_diff()

        # Determine whether to run external tools for this invocation.
        if enable_external_tools is None:
            external_enabled = self.enable_external_tools
        else:
            external_enabled = enable_external_tools

        # Resolve spec path preference for the encoding scan helper.
        spec_to_use = spec_path if spec_path is not None else self.spec_path
        ...
        # Optionally attach sidecar results from external verification tools.
        if external_enabled:
            external: Dict[str, Any] = {}
            external["deepseek_verifier"] = run_deepseek_verifier(pr_url)
            external["gpt52_pr_scanner"] = run_gpt52_scanner(pr_url)
            external["encoding_scan"] = run_encoding_scan(spec_to_use)
            results["external_tools"] = external

        return results

    def _pass_1_amendment14(self, pr_url: str) -> Dict[str, Any]:
        ...
        if self.parser:
            ...
            if amendment_scan["found"]:
                ...
                for comp, present in amendment_scan["components"].items():
                    idx = list(amendment_scan["components"].keys()).index(comp) + 1
                    key = f"component_{idx}_{comp}"
                    pass_result["checks"][key] = present
                pass_result["details"].append(
                    f"✓ Amendment #14 found with {amendment_scan['component_count']}/7 components",
                )
                pass_result["status"] = (
                    "SUCCESS" if amendment_scan["component_count"] == 7 else "INCOMPLETE"
                )
                if amendment_scan["component_count"] < 7:
                    self.red_flags.append(
                        f"Amendment #14 incomplete: only {amendment_scan['component_count']}/7 components",
                    )
            else:
    ...
    def _pass_2_probe_links(self, pr_url: str) -> Dict[str, Any]:
    ...
    def _pass_3_components(self, pr_url: str) -> Dict[str, Any]:
    ...
    def _pass_4_backward_compat(self, pr_url: str) -> Dict[str, Any]:
        ...
        if self.parser:
            ...
            pass_result["details"].append(
                f"✓ Found {compat_scan['markers_found']}/{compat_scan['total_markers']} backward compatibility markers",
            )
            pass_result["status"] = (
                "SUCCESS" if compat_scan["markers_found"] >= 4 else "INCOMPLETE"
            )
            if not compat_scan["markers"]["recommended_not_required"]:
                self.red_flags.append("Missing 'RECOMMENDED not REQUIRED' language")
        else:
            pass_result["details"].append(
                "⚠️ Scanning for backward compatibility language markers",
            )
            pass_result["status"] = "PENDING"

        return pass_result

    def _pass_5_kappa_classification(self, pr_url: str) -> Dict[str, Any]:
        ...
        if self.parser:
            ...
            pass_result["details"].append(
                "⚠️ Checking for κ=1.0 and Mixed-Hybrid classification",
            )
            pass_result["status"] = "PENDING"
